#![feature(box_patterns, core_intrinsics, const_type_name)]
...
fn output_code(file_name: String, entries: &Vec<Entry>) {
...
fn main() {
    ...
    let entries = assign_codes(symbol_counts.pop().expect("No nodes in tree").node, None);
    assert!(
        symbol_counts.pop().is_none(),
        "Tree contained more than one root"
    );
    output_code("codes.rs".to_string(), &entries);
