...
impl<T: FileTy> File<T> {
    ...
    pub fn close<'f>(self) -> Result<u8, Error<'f, T>> {
        let res = unsafe { kernel::file_close(self.fd) };
        forget(self);
        match res {
            i8::MIN..=-2 => Err(Error::Resolved(ErrorKind::UnknownError)),
            -1 => Err(Error::Unresolved),
            0..=i8::MAX => Ok(res as u8),
        }
    }
...
impl File<MemCard> {
    ...
    // TODO: The BIOS stores the paths in RAM so this can be impl'ed w/o a `path`
    // field in `File` pub fn rename<P: AsRef<[u8]>>(&mut self, new_name: P) ->
    // Result<(), Error<MemCard>> {    new_name.as_ref().as_cstr(|new_name| {
    //        let res =
    //            unsafe { kernel::file_rename(todo!("self.path.as_ptr()"),
    // new_name.as_ptr()) };        Self::try_change_path(res)
    //    })
    //}
    //pub fn delete(&mut self) -> Result<(), Error<MemCard>> {
    //    let res = unsafe { kernel::file_delete(todo!("self.path.as_ptr()")) };
    //    Self::try_change_path(res)
    //}
    //pub fn recover(&mut self) -> Result<(), Error<MemCard>> {
    //    let res = unsafe { kernel::file_undelete(todo!("self.path.as_ptr()")) };
    //    Self::try_change_path(res)
    //}
    //fn try_change_path<'f>(res: u8) -> Result<(), Error<'f, MemCard>> {
    //    match res {
    //        0 => Err(Error::Unresolved),
    //        1 => Ok(()),
    //        2..=u8::MAX => Err(Error::Resolved(ErrorKind::UnknownError)),
    //    }
    //}
}
