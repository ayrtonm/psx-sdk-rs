...
pub trait Register<T: Primitive>: HasValue<T> + Read<T> {
    ...
    /// Checks if all of the given `bits` are cleared in the handle's copy of
    /// the register.
    fn cleared(&self, bits: T) -> bool {
        self.get() & bits == T::from(0)
    }
}

pub trait MutRegister<T: Primitive>: Sized + Register<T> + Write<T> {
    ...
    fn clear(&mut self, bits: T) -> &mut Self {
        *self.get_mut() &= !bits;
        self
    }

    /// Toggles the given `bits` in the handle's copy of the register.
    fn toggle(&mut self, bits: T) -> &mut Self {
        *self.get_mut() ^= bits;
        self
    }
