macro_rules! impl_primitive {
    ($name:ident, $alloc_fn:ident, $array_alloc_fn:ident, $cmd:expr) => {
        ...
        impl<const N: usize> Buffer<N> {
            ...
            /// insufficient.
            pub fn $alloc_fn(&self) -> Option<Ref<$name>> {
                self.packet()
            }

            /// Allocates an array of packets. Returns `None` if remaining buffer space
            /// is insufficient.
            pub fn $array_alloc_fn<const M: usize>(&self) -> Option<[Ref<$name>; M]> {
                self.packet_array()
            }
        }
        ...
        impl<const N: usize> DoubleBuffer<N> {
            ...
            pub fn $alloc_fn(&self) -> Option<DoubleRef<$name>> {
                self.packet()
            }

            /// Allocates an array of double-buffered packets. Returns `None` if
            /// remaining buffer space is insufficient.
            pub fn $array_alloc_fn<const M: usize>(&self) -> Option<[DoubleRef<$name>; M]> {
                self.packet_array()
            }
        }
    };
