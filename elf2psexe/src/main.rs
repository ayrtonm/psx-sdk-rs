...
impl ElfReader {
    ...
    fn parse(&mut self) {
        ...
        self.sections.sort_by(|s1, s2| s1.base.cmp(&s2.base));
    ...
    fn base(&self) -> u32 {
        // Sections are sorted by base address so we now we get the
        // lowest base address in the first entry
        self.sections[0].base
    }
}
...
fn main() {
    ...
    if args.len() < 2 {
        panic!("usage: elf2psexe <elf-bin>");
    }

    let elfpath = &args[1];

    let elf = ElfReader::new(Path::new(elfpath));

    println!("Entry point: {:08x}", elf.entry());
    println!("Base: {:08x}", elf.base());
}
